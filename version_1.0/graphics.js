class Graphics {
	...
	loadMatrix(matrix_){
		this.privateMatrix = matrix_;
		this.privateDimentions = [matrix_[0].length, matrix_.length]
		this.update();
	}

	update(){
	    if(!(Array.isArray(this.privateMatrix) && this.privateMatrix.length > 0 
	    	&& Array.isArray(this.privateMatrix[0]) && this.privateMatrix[0].length > 0))
	        return;
	    this.#privateC_Context.clearRect(0, 0, this.#privateC.width, this.#privateC.height);
	    this.#privateC_Context.beginPath();
		this.fillGrid();
	}

	fillGrid(){
		let binary = this.privateMatrix.some(item => item.some(item => item !== 0  && item != 1));
		for(let c=0; c<this.privateDimentions[0]; c++){
			for(let r=0; r<this.privateDimentions[1]; r++){
				if(binary)
					this.fillCell(c,r,this.privateDimentions, this.privateMatrix[r][c])
				else
					this.fillCell(c,r,this.privateDimentions, this.privateMatrix[r][c] == 0 ? 'white' : 'black')
